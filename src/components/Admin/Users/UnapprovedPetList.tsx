...
import useUnapprovedPetsQuery from "@/hooks/useUnapprovedPetsQuery";
import { adminService } from "@/services/adminService";
import { PetFilters, SortDirection } from "@/types/petListing";
import { useQueryClient } from "@tanstack/react-query";
import { CheckCheck, ChevronDown, CircleCheck } from "lucide-react";
...
const UnapprovedPets = () => {
  ...
  const approvePets = async () => {
    if (selectedIds.length === 0) return;
    const res = await adminService.approvePets({ ids: selectedIds });
    if (res.success) {
      queryClient.invalidateQueries({ queryKey: ["unapproved"] });
      setSelectedIds([]);
    ...
    <div className="min-h-screen bg-gray-100 text-gray-900 p-6">
      ...
      <div className="flex flex-col item-start gap-6 justify-between mb-6">
        <div className="flex w-full items-center justify-between py-2 ">
          <h1 className="text-2xl ml-4 font-bold">Unapproved Pets</h1>
          <SortDropdown
      ...
      <div className="overflow-x-auto rounded-lg shadow">
        <table className="w-full bg-white border border-gray-200 rounded-lg">
          ...
          <tbody>
            {unapprovedPets?.data?.content.map((pet, index) => (
              <tr
                key={pet.id}
                className={`border-b ${
      ...
      {unapprovedPets?.data && (
        <Pagination
          ...
          currentPage={(unapprovedPets?.data.pageNumber ?? 0) + 1}
          totalPages={unapprovedPets?.data.totalPages ?? 1}
          onPageChange={(page) => handlePageChange(page - 1)}
        />
      )}
      <button
        onClick={() => {
          if (unapprovedPets && unapprovedPets.data)
            setSelectedIds(
              unapprovedPets.data?.content?.map((n) => Number(n.id))
            );
          else return;
        }}
        className={
      ...
      <button
        ...
        className={
          selectedIds.length > 0
            ? "fixed bottom-6 right-6 w-14 h-14 flex items-center justify-center rounded-full text-white bg-green-500 shadow-lg hover:bg-green-600 transition-all"
            : "hidden"
        }
      >
        <CircleCheck size={40} />
